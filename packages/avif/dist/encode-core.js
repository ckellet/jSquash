...
export function createEncoder(loadCodec) {
    ...
    async function encode(data, options = {}) {
        ...
        const icc = _options.icc === undefined ? undefined : toIccProfileBytes(_options.icc);
        ...
        const pixels = new Uint8Array(data.data.buffer, data.data.byteOffset, data.data.byteLength);
        return codecModule.use((codec) => {
            const output = withPixelBuffer(codec, pixels, (pointer) => icc === undefined
                ? codec.encode(pointer, data.width, data.height, _options)
                : withPixelBuffer(codec, icc, (iccPointer) => codec.encode_with_icc(pointer, data.width, data.height, _options, iccPointer, icc.byteLength)));
            if (!output) {
