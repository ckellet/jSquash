...
const codecModule = createModuleCache(() => mozjpeg_enc);
...
export default async function encode(data, options = {}) {
    ...
    return codecModule.use((codec) => {
        ...
        const resultView = withPixelBuffer(codec, data.data, (pointer) => icc === undefined
            ? codec.encode(pointer, data.width, data.height, _options)
            : withPixelBuffer(codec, icc, (iccPointer) => codec.encode_with_icc_profile(pointer, data.width, data.height, _options, iccPointer, icc.byteLength)));
        if (!resultView)
